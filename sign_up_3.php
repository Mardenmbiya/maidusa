<?php include('base.php'); ?>
<?php include("nave_bar_single.php") ?>
 <div class="marge_generale"></div>
    <div class="container_login_2" >
        <div class="container_login_21" >
                <div class="container_login_211">
                     <img src="images/logo-2.png" alt="">
                </div>
               
                <div class="container_login_212">
                    <form action="">
                       <input type="password" name="" placeholder="Mot de passe" id="" class="form_design2">
                       <input type="password" name="" id="" placeholder="Confirmer le mot de passe" class="form_design2">
                    </form>
                     <div class="cont_log1">
                         <a href="Acceuil.php"><button class="button_1">S'inscire</button></a>
                     </div>
                     <div class="cont_log2">
                         <a href="login.php"><div class="txt3 color_1 hover_all">Déja un compte? Se connecter</div></a>
                     </div>
                     
                </div>
                 
                
        </div>
     </div>
</body>
</html>